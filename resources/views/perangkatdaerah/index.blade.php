            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="bg-white border-b border-gray-200">
                    ...
                    <table class="table-fixed w-full mt-4">
                        ...
                        @php($no=$i)
                        @forelse($perangkat_daerahs as $perangkat_daerah)
                            @if($no % 2 == 0)
                            ...
                            @endif
                                ...
                                <td class="px-2 py-2 text-gray-500">
                                    <div class="flex space-x-2 float-right">
                                        <button onclick="window.location='{{ route('perangkat_daerah.show', $perangkat_daerah->id) }}'" class="border-2 border-black-200 rounded-md p-1 hover:bg-green-300" title="Lihat">
                                        ...
                                        <button onclick="window.location='{{ route('perangkat_daerah.edit', $perangkat_daerah->id) }}'" class="border-2 border-black-200 rounded-md p-1 hover:bg-green-300" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="h-4 w-4 text-gray-500">
                    ...
                    <hr/>
                    <div class="m-3">{{ $perangkat_daerahs->links() }}</div>
                </div>
            </div>
        </div>
    </div>
